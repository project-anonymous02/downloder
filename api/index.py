...
async def _fetch_search_results(query: str) -> list[str]:
    """Ambil daftar URL dari DuckDuckGo HTML search."""
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
            ...
            # ── FIX: DDG menyimpan URL asli di parameter uddg= (URL-encoded) ──
            uddg_raw = re.findall(r'uddg=(https?[^&"\'>\s]+)', html, re.I)
            for raw in uddg_raw:
            ...
            # ── Fallback: href langsung non-DDG ───────────────────────────────
            if not urls:
                raw_links = re.findall(
                    r'href=["\']?(https?://(?!duckduckgo\.com)[^"\'>\s]+)',
                    html, re.I,
                )
                for u in raw_links:
                    u = u.strip().rstrip("/")
...
async def _fetch_page_data(url: str, timeout: int = 8) -> dict:
    """Fetch satu halaman dan ekstrak title + teks bersih."""
    hdrs = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                      "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Accept":     "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
...
def _score_relevance(query: str, title: str, text: str) -> float:
    """Skor relevansi: judul (3×) + konten (1×) + bonus semua keyword di judul."""
    ...
    for kw in keywords:
        score += title_lower.count(kw) * 3.0
        score += min(text_lower.count(kw), 20) * 1.0
...
def _make_summary(text: str, query: str, max_chars: int = 1000) -> str:
    """Ringkasan ekstraktif, prioritas kalimat ber-keyword. Maks 1000 karakter."""
    if not text:
        return ""
    ...
    for idx, sent in enumerate(sentences):
        s_lower   = sent.lower()
        hits      = sum(s_lower.count(kw) for kw in keywords)
        pos_bonus = max(0, 5 - idx) * 0.3
        scored.append((hits + pos_bonus, idx, sent))
    ...
    for _, idx, sent in scored:
        if total + len(sent) + 1 > max_chars:
            ...
            if remaining > 40:
                selected.append((idx, sent[:remaining].rsplit(" ", 1)[0] + "…"))
            break
...
@app.get("/api/search")
async def search_endpoint(summary: str = ""):
    """GET /api/search?summary=<query>"""
    query = summary.strip()
    if not query:
        return JSONResponse({
            "status":             False,
    ...
    # 1. Ambil URL dari DDG
    search_urls = await _fetch_search_results(query)
    if not search_urls:
        return {
            "query":              query,
            "search_url":         [],
    ...
    # 2. Fetch semua halaman paralel
    pages_raw = await asyncio.gather(
        *[_fetch_page_data(url) for url in search_urls],
        return_exceptions=True,
    )
    pages: list[dict] = [
        item if isinstance(item, dict)
        else {"url": "", "title": "", "text": "", "ok": False}
        for item in pages_raw
    ]

    # 3. Scoring
    scored_pages: list[tuple[float, dict]] = []
    for page in pages:
        if not page.get("ok") or not page.get("url"):
            scored_pages.append((0.0, page))
            continue
        score = _score_relevance(query, page.get("title", ""), page.get("text", ""))
        scored_pages.append((score, page))

    scored_pages.sort(key=lambda x: -x[0])

    # 4. Pilih top-5, indeks 1-based mengacu ke search_url
    url_to_idx          = {url: i + 1 for i, url in enumerate(search_urls)}
    ...
    for _, page in scored_pages:
        ...
        if not url or not page.get("ok"):
            continue
        orig_idx = url_to_idx.get(url)
        if orig_idx is None:
            continue

        recommended_indices.append(orig_idx)
        search_data.append({
            "url":     url,
            "title":   page.get("title", "") or url,
            "summary": _make_summary(page.get("text", ""), query, max_chars=1000),
        })

    return {
        "query":              query,
        "search_url":         search_urls,
        "search_recommended": recommended_indices,
