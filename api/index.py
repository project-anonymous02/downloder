...
async def activity_middleware(request: Request, call_next):
    ...
    # Baca body agar tetap bisa dibaca endpoint asli
    ...
    ev.run_in_executor(None, _activity_append, entry)
    asyncio.create_task(_discord_notify(entry))
...
async def _search_fetch_ddg(query: str) -> list[str]:
    """Ambil URL dari DuckDuckGo HTML. Return [] jika gagal."""
    ...
    try:
        async with httpx.AsyncClient(timeout=12.0, follow_redirects=True) as client:
            ...
            seen: set  = set()
            urls: list = []
            # Pattern utama: uddg= (URL asli DDG yang di-encode)
            for raw in re.findall(r'uddg=(https?[^&"\'>\s]+)', html, re.I):
                ...
                if len(urls) >= 25:
                    break
            # Fallback jika uddg tidak ditemukan
            if not urls:
...
async def _search_fetch_page(url: str) -> dict:
    """Fetch satu halaman. Selalu return dict, tidak pernah raise exception."""
    hdrs = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                      "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Accept":     "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
...
def _search_score(query: str, title: str, text: str) -> float:
    """Skor relevansi keyword di judul (3x) dan konten (1x)."""
    try:
...
def _search_summary(text: str, query: str, max_chars: int = 1000) -> str:
    """Ringkasan ekstraktif maks 1000 karakter. Tidak pernah raise exception."""
    try:
        if not text:
            return ""
        keywords  = [w.lower() for w in re.split(r"\W+", query) if len(w) > 1]
...
async def _search_core(query: str) -> dict:
    """Logic utama search. Selalu return dict valid, tidak pernah raise."""
    try:
        empty = {
            "query":              query,
            "search_url":         [],
        ...
        # 2. Fetch semua halaman paralel, max 7s per halaman
        ...
        # 4. Top-5 recommended (indeks 1-based ke search_url)
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            if method == "POST":
                ...
                # endpoint bisa sudah mengandung query string sendiri
                # params dict hanya di-merge jika ada isinya
                if params:
        ...
        # Ambil status dari field "status" di body jika ada, fallback ke HTTP code
...
async def multiple_get(tasks: str = ""):
    ...
    # Setiap item adalah full path termasuk query string-nya sendiri
    coros = [
...
async def multiple_post(request: Request):
    ...
    # Validasi dan siapkan task list
    # Tiap item: ("ok", task_id, endpoint, params, method) atau dict error langsung
    prepared: list = []
    ...
    async def _dispatch(item) -> dict:
        if isinstance(item, dict):
            # sudah jadi error result, kembalikan langsung
            return item
    ...
    # Urutkan sesuai task_id agar selalu konsisten
...
async def rum_activity_get(
    ...
    """RUM activity tracking - GET endpoint"""
    entry = {
...
async def rum_activity_post(request: Request):
    """RUM activity tracking - POST endpoint"""
    try:
        body = await request.json()
    except Exception:
        return JSONResponse({"status": False, "error": "Body harus JSON yang valid"})
