...
@app.get("/api")
@app.get("/api/run")
async def god_mode_get(code: str = "", status: str = ""):
    if status:
        return _loop_get_status(status)
    if not code:
        ...
        if raw_data:
            if isinstance(raw_data, dict):
                ...
                try:
                    ...
                    return JSONResponse({"status": False, "error": "Field 'data' bukan JSON yang valid"})
...
def _loop_status_path(loop_id: str) -> str:
    return os.path.join(LOOP_DIR, f"loop_{loop_id}.json")


def _loop_write_status(loop_id: str, data: dict):
    try:
        with open(_loop_status_path(loop_id), 'w') as f:
            json.dump(data, f, indent=2)
    except Exception:
        pass


def _loop_get_status(loop_id: str):
    path = _loop_status_path(loop_id)
    if not os.path.exists(path):
        return JSONResponse({"status": False, "error": f"Loop '{loop_id}' tidak ditemukan"})
    try:
        with open(path, 'r') as f:
            data = json.load(f)
        with _loop_lock:
            data["in_registry"] = loop_id in _loop_registry
        return {"status": True, "loop": data}
    except Exception as e:
        return JSONResponse({"status": False, "error": str(e)})


def _loop_stop(loop_id: str):
    with _loop_lock:
        entry = _loop_registry.get(loop_id)
    if not entry:
        path = _loop_status_path(loop_id)
        if os.path.exists(path):
            return {"status": True, "message": "Loop sudah berhenti", "loop_id": loop_id}
        return JSONResponse({"status": False, "error": f"Loop '{loop_id}' tidak ditemukan"})
    entry["stop_event"].set()
    return {"status": True, "message": "Stop signal dikirim", "loop_id": loop_id}


def _loop_start(code: str, interval: int) -> dict:
    ...
    def run():
        ...
        _loop_write_status(loop_id, status_data)

        while not stop_event.is_set():
            iteration += 1
            ts = datetime.utcnow().isoformat() + "Z"
            result = _run_god_mode(code)
            ...
            status_data["last_heartbeat"] = ts
            status_data["last_output"] = result.get("output", "")
            status_data["last_error"] = result.get("error")
            status_data["history"] = (status_data["history"] + [entry])[-20:]

            if not result.get("status"):
                status_data["status"] = "error"
                _loop_write_status(loop_id, status_data)
                with _loop_lock:
                    _loop_registry.pop(loop_id, None)
                return
        ...
        status_data["stopped_at"] = datetime.utcnow().isoformat() + "Z"
        _loop_write_status(loop_id, status_data)
        with _loop_lock:
            _loop_registry.pop(loop_id, None)
    ...
    with _loop_lock:
        _loop_registry[loop_id] = {"thread": t, "stop_event": stop_event}
    t.start()
...
async def loop_get(
    ...
    if action == "stop" and loop_id:
        return _loop_stop(loop_id)
    if action == "status" and loop_id:
        return _loop_get_status(loop_id)
    if action == "list":
        return _loop_list()
    if not code:
        return JSONResponse({"status": False, "error": "Parameter 'code' wajib diisi"})
    return _loop_start(code, interval)
...
def _loop_list() -> dict:
    try:
        files = [f for f in os.listdir(LOOP_DIR) if f.startswith("loop_") and f.endswith(".json")]
        loops = []
        for f in files:
            try:
                with open(os.path.join(LOOP_DIR, f), 'r') as fp:
                    data = json.load(fp)
                with _loop_lock:
                    data["in_registry"] = data.get("loop_id", "") in _loop_registry
                loops.append(data)
            except Exception:
                pass
...
@app.post("/api/fetch")
async def fetch_url_post(request: Request):
    try:
...
async def nova_webhook_send_post(request: Request):
    ...
    if not target_url.startswith(("http://", "https://")):
        return JSONResponse({"status": False, "error": "Field 'url' harus URL valid (http/https)"})
    ...
    try:
        async with httpx.AsyncClient(timeout=float(timeout)) as client:
            if method == "GET":
                res = await client.get(target_url, params=payload if isinstance(payload, dict) else {}, headers=headers)
            else:
        ...
        return JSONResponse({"status": False, "error": f"Timeout setelah {timeout} detik"})
    except httpx.RequestError as e:
        return JSONResponse({"status": False, "error": f"Request error: {str(e)}"})
    except Exception as e:
        return JSONResponse({"status": False, "error": str(e)})
...
async def nova_webhook_send_get(
    ...
    if not url:
        return JSONResponse({"status": False, "error": "Parameter 'url' wajib diisi"})
    if not url.startswith(("http://", "https://")):
        return JSONResponse({"status": False, "error": "Parameter 'url' harus URL valid (http/https)"})
    ...
    if data:
        try:
            ...
            if isinstance(parsed, dict):
                ...
                return JSONResponse({"status": False, "error": "Parameter 'data' harus berupa JSON object"})
        except json.JSONDecodeError:
            return JSONResponse({"status": False, "error": "Parameter 'data' bukan JSON yang valid"})
...
# ==========================================
# 7. WEBHOOK MANAGEMENT SYSTEM (/wh/)
# ==========================================
...
async def wh_delete_post(request: Request):
    try:
        ...
        return JSONResponse({"status": False, "error": "Request body harus berupa JSON yang valid"})
    loop   = asyncio.get_running_loop()
    result = await loop.run_in_executor(None, _do_webhook_delete, (body.get("id") or "").strip())
    return JSONResponse(result) if not result.get("status") else result
