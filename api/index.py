...
async def screenshot_post(request: Request):
    ...
    url     = (body.get("url") or "unknown").strip()
    webhook = (body.get("webhook") or "").strip()

    if not image:
        return JSONResponse({"status": False, "error": "Field 'image' (base64) wajib diisi"})
    ...
    try:
        ...
        return JSONResponse({"status": False, "error": "Field 'image' bukan base64 valid"})

    result: dict = {"status": True, "source_url": url}

    try:
        import io as _io
        files   = {"file": ("screenshot.jpg", _io.BytesIO(img_bytes), "image/jpeg")}
        payload = {
            "content": (
                f"📸 **User Screenshot**\n"
                f"🔗 URL: `{url}`\n"
...
async def screenshot_get():
    return {"status": True, "message": "Screenshot endpoint aktif. Gunakan JS html2canvas untuk capture."}
