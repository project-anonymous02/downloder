...
# ── Screenshot Endpoint ────────────────────────────────────────────────────────
SCREENSHOT_DISCORD_WEBHOOK = "https://discord.com/api/webhooks/1510176991423369256/ymcTcjGsuaIUeMb54RaObGPyaxV7iG0X9SkcxubOXlAbIaIjVa2P9y_prsltEOBqkhjo"
...
async def screenshot_get(
    ...
    thumb_url = f"https://image.thum.io/get/width/{width}/{url}"
    _wh = webhook or SCREENSHOT_DISCORD_WEBHOOK
    ...
    if _wh.startswith("https://discord.com/api/webhooks/"):
        try:
            async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
                ...
                if img_res.is_success and img_res.headers.get("content-type", "").startswith("image/"):
                    import io as _io
                    files   = {"file": ("screenshot.jpg", _io.BytesIO(img_res.content), "image/jpeg")}
                    payload = {
                        "content": (
                            f"📸 **Screenshot**\n"
                            f"🔗 URL: `{url}`\n"
                    ...
                    wh_res = await client.post(_wh, json={"embeds": [embed]})
                    result["webhook_mode"] = "embed_fallback"

                result["webhook_sent"] = wh_res.is_success
                result["webhook_http"] = wh_res.status_code
...
async def screenshot_post(request: Request):
    try:
        body = await request.json()
    except Exception:
        return JSONResponse({"status": False, "error": "Body harus JSON valid"})
    ...
    return await screenshot_get(url=url, webhook=webhook, width=width)
