...
async def god_mode_post(request: Request):
    """
    Mode 1 — God Mode  : POST {"code": "print('hi')"}
    Mode 2 — Fetch Proxy: POST {"url": "https://...", "method": "POST", "data": {...}, "headers": {...}}
    """
    try:
    ...
    # Mode 2: Fetch Proxy — jika ada field 'url'
    if "url" in body:
    ...
    # Mode 1: God Mode — jika ada field 'code'
...
# ==========================================
# 2-A. PROXY FETCH — GET
# Contoh kirim POST ke URL lain:
# GET /api/fetch?url=https://api.example.com&method=POST&body={"key":"val"}
# ==========================================
@app.get("/api/fetch")
...
async def receive_or_list_webhook_get(request: Request, token: str = "", list: int = 0):
    """
    GET /nova/webhook?list=1          → list semua file webhook
    GET /nova/webhook?token=x&list=1  → list webhook by token
    GET /nova/webhook?token=x&foo=bar → simpan query params sebagai data webhook
    """
    if list == 1:
        try:
            webhook_dir = os.path.join(STORAGE_DIR, 'webhooks')
...
async def nova_webhook_send_post(request: Request):
    """
    Kirim webhook ke URL lain via POST.
    Body: {"url": "https://...", "data": {...}, "secret": "optional", "method": "POST"}
    """
    try:
        body = await request.json()
    except Exception:
...
async def nova_webhook_send_get(
    ...
    """
    Kirim webhook via GET query params.
    GET /nova/webhook/send?url=https://...&data={"key":"val"}&method=POST
    """
    if not url:
...
# ==========================================
# 7. WEBHOOK MANAGEMENT SYSTEM
# PINDAH KE /wh/ — fix 404 di Vercel karena
# /api/ prefix conflict dengan serverless functions
# ==========================================
...
# --- /wh/list ---
@app.get("/wh/list")
# Tetap tersedia di path lama juga
