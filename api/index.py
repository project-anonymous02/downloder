...
@app.get("/")
async def root():
    return {"status": "online", "message": "Server Nova aktif bestie! 🫂💙", "endpoints": ["/api", "/api/fetch", "/api/doc", "/api/image", "/api/webhook", "/api/qr"]}
...
async def process_doc(action: str, url: str, page: int = 0):
    ...
    try:
        ...
        return JSONResponse({"status": False, "error": str(e)})
    finally:        if temp_filepath and os.path.exists(temp_filepath):
...
async def process_image(action: str, url: str, width: int = 0, height: int = 0, format: str = ""):
    ...
    try:
        ...
        if action == 'info':
            ...
            output_filepath = f"/tmp/{uuid.uuid4().hex}.{format or img.format.lower() or 'png'}"
            img.save(output_filepath)
            with open(output_filepath, 'rb') as f:
                data = f.read()
            return Response(content=data, media_type=f"image/{format or img.format.lower() or 'png'}")
        elif action == 'convert':
            if not format:
