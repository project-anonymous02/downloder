...
class handler(BaseHTTPRequestHandler):
    ...
    def _run_python_code(self, code):
        ...
        try:
            ...
            self._send_json(200, {'status': True, 'mode': 'python', 'output': mystdout.getvalue(), 'stderr': mystderr.getvalue() or None, 'code': code})
        except Exception as e:
            self._send_json(400, {'status': False, 'mode': 'python', 'error': str(e), 'detail': traceback.format_exc(), 'stderr': mystderr.getvalue() or None, 'code': code})        finally:
        ...
        for cmd in commands:
            ...
            try:
                if is_shell:
                    ...
                    if blocked:
                        res.update({'status': False, 'error': 'Blocked'})
                    else:
                        r = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=max_t, cwd='/tmp')                        res.update({'status': True, 'output': r.stdout, 'stderr': r.stderr, 'exit_code': r.returncode})
                else:
