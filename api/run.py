...
# ========================================
# Ganti ini dengan Environment Variable di Vercel Dashboard (Settings > Environment Variables)
# Nama variable: API_SECRET. Kalau kosong, berarti mode publik (nggak aman).
API_SECRET = os.environ.get("API_SECRET", "")
STORAGE_DIR = "/tmp/nova_storage"
os.makedirs(STORAGE_DIR, exist_ok=True)

# Blacklist command shell biar nggak nge-hang atau ngerusak sistem
SHELL_BLACKLIST = [
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Validasi Auth (Cek header atau query ?token=)
        ...
        self._process_request(action, code, sh_flag, timeout, payload, is_post=False)
    def do_POST(self):
        # Validasi Auth
        ...
        self._process_request(action, code, sh_flag, timeout, payload, is_post=True)
    ...
    # ========================================
    def _process_request(self, action, code, sh_flag, timeout, payload, is_post):
        ...
        try:
            if action == 'storage':
                ...
                else:                    self._run_python_code(code)
            else:
                self._send_json(400, {'status': False, 'error': f'Action "{action}" tidak dikenali. Gunakan: run, batch, storage.'})
    ...
    def _run_shell_command(self, cmd, timeout):
        # Validasi & Sanitasi (Cek Blacklist)
        ...
        try:
            # Batasi timeout maksimal 55 detik (Vercel Pro) atau 9 detik (Free)
    ...
    # ========================================
    def _run_python_code(self, code):        old_stdout, old_stderr = sys.stdout, sys.stderr
        sys.stdout = mystdout = StringIO()
        sys.stderr = mystderr = StringIO()
        
        # Inject environment info ke globals python
        exec_globals = {
        ...
        try:
            ...
            # FIX BUG: NameError 'code' is not defined
            self._send_json(400, {
                'status': False,
                'mode': 'python',
                'error': str(e),
...
Waduh, bestie! 🫂 Makasih banget udah nge-review dan nyatet bugs serta fitur yang kurang. Kamu emang jeli banget, dan catatan kamu itu **daging semua**! 🔥 

Tenang, kita beresin semuanya sekarang juga. Sesuai permintaan kamu, kita **nggak perlu bikin endpoint baru** (kayak `/api/batch` atau `/api/storage`). Kita bakal sulap `/api/run` jadi **Satu Endpoint Maha Kuasa (God Mode)** yang nanganin semua fitur di atas pakai parameter `action`. 

Ini dia versi **Final & Ultimate** dari `/api/run.py`. Bugs diperbaiki, security ditambah, dan fitur-fitur advanced langsung masuk! Gas pol! 🚀

---

### 🛠️ UPDATE FILE: `/api/run.py` (Versi Ultimate All-in-One)

Hapus semua isi `/api/run.py` yang lama, ganti pakai kode monster ini:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        # Validasi Auth
        token = self.headers.get('Authorization', '').replace('Bearer ', '')
        if API_SECRET and token != API_SECRET:            return self._send_json(401, {'status': False, 'error': 'Unauthorized. Token salah atau kosong.'})
    ...
    def _handle_storage(self, cmd, payload):
        if cmd == 'save' and payload:
            ...
            if os.path.exists(filepath):
                with open(filepath, 'rb') as f:                    content = base64.b64encode(f.read()).decode('utf-8')
    ...
    # ========================================
    # ⚡ 4. BATCH EXECUTOR (Multiple Commands)
    # ========================================
    def _handle_batch(self, code, timeout):
        try:
            # code di sini berisi JSON string array of commands
        ...
        results = []
        max_timeout_per_cmd = max(1, min(timeout, 55) // len(commands)) # Bagi rata timeout
